/** @typedef {{id: string, practice: string, attemptedAt: string, obtained: number, total: number, mcq: number, saqs: number, laqs: number}} AnalyticsAttempt */
...
export function buildCompetencyAnalytics(card = {}) {
  ...
  return { code: card.competencyCode || '', name: card.competencyName || '', practiceCount: sessions.length, attempts }
}
...
export const competencyAnalyticsSample = {
  ...
  practiceSessions: [{ id: 'sample', practiceNo: 1, mcq: 1, saqs: 1, laqs: 0, practiceAttemptHistory: [
...
export async function getCompetencyAnalytics(card) {
  return buildCompetencyAnalytics(card)
}
