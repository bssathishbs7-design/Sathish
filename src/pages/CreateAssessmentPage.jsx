...
export default function CreateAssessmentPage({ onNavigate, theme = 'light', onToggleTheme }) {
  ...
  const descriptiveValidationRows = currentDescriptiveSections.flatMap((section) => {
    const children = section.children ?? []
    return children.length ? children : [section]
  })
  const areDescriptiveRowsReady = descriptiveValidationRows.length
    ? descriptiveValidationRows.every((row) => (
  ...
  const canCreate = isMcqQuestion
    ...
    : isDescriptiveCurrentQuestion
      ? hasQuestionText && (currentDescriptiveSections.length ? areDescriptiveRowsReady : requiredProgress === 6)
  ...
  const renderDescriptiveCompetencyButton = (target, item, disabled = false) => {
    ...
    if (!selectedCompetency) {
      return (
        <button
          type="button"
          className="question-bank-secondary-btn question-bank-descriptive-competency-btn"
                      ...
                      <div className="question-bank-descriptive-builder-foot">
                        <button
                          ...
                          disabled={!getRichTextPreview(question.questionText) || (() => {
                            ...
                            return lastChild ? !getRichTextPreview(lastChild.questionText) : !getRichTextPreview(lastSection.questionText)
                          })()}
                        >
                          <Plus size={14} strokeWidth={2.2} />
                          Add Sub Question
