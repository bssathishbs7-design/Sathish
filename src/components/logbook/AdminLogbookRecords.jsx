...
export default function AdminLogbookRecords({ rows, onOpen, grouped = false, eligible = [], checked = [], onCheck, onCancel, actorId, empty = 'No entries in this view.' }) {
  return <div className="admin-entry-list">{!rows.length && <p className="lb-empty">{empty}</p>}{rows.map(entry => <div className="admin-entry" key={entry.id}>
    ...
    <button className="admin-entry-open" onClick={() => onOpen(entry.id)}><span><strong>{entryTitle(entry)}</strong><small>{[!grouped && learnerName(entry.studentId), entry.subject, categoryLabel(entry.cat)].filter(Boolean).join(" / ")}</small><small>{entry.status === 'Pending' ? `${waitingDays(entry)} days waiting${overdueEntry(entry) ? ' · Overdue' : ''}${isGraded(entry) ? ' · Needs grade' : ''}` : entry.assignment?.due ? `Due ${formatDate(entry.assignment.due)}` : ''}</small></span><span className="admin-entry-meta"><span className={`lb-status is-${entry.status.toLowerCase().replaceAll(' ', '-')}`}>{entry.status}</span><small>{formatDate(entry.submittedAt || entry.date)}</small><span className="admin-record-flags">{entry.extra?.attachments?.length > 0 && <Paperclip size={13} aria-label="Has attachments" />}{(entry.extra?.comments?.length > 0 || entry.extra?.remarks) && <MessageSquare size={13} aria-label="Has comments or remarks" />}</span></span>{grouped ? <span className="admin-queue-review">Review<ChevronRight size={14} aria-hidden="true" /></span> : <ChevronRight size={16} />}</button>
