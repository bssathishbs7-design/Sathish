...
import { ArrowRight, Check, LoaderCircle, LockKeyhole, Paperclip, Plus, Trash2 } from 'lucide-react'
...
export default function LogbookEntryForm({ entry, entries = [], mode, theme, onSaved, onClose }) {
  ...
  const field = (key, definition = getLogbookField(form.cat, key, form.subject)) => {
    ...
    return <label className={`lb-field ${definition.wide ? 'lb-field-wide' : ''}`} key={key} htmlFor={`lb-${key}`}>
      <span>{definition.label}{definition.required && <span className="lb-required" aria-hidden="true"> *</span>}</span>
      {definition.type === 'select' ? <select {...props}><option value="">Choose one</option>{props.value && !definition.options.includes(props.value) && <option value={props.value}>{props.value} (previous value)</option>}{definition.options.map(option => <option key={option}>{option}</option>)}</select>
      ...
      <fieldset disabled={busy} className="lb-entry-scroll">
        ...
        <LogbookAccordionSection index={1} title="Activity details" summary={detailsReady ? (form.values.activity || form.values.topic || 'Required details completed') : contextReady ? 'Complete the fields for your selected category' : 'Select a subject and category first'} open={step === 1} complete={detailsReady} disabled={!contextReady} onOpen={() => openStep(1)}>
          ...
          {groups.map(group => <section className="lb-schema-group" key={group.title} aria-label={group.title}>
            ...
            <div className="lb-form-grid">{group.fields.map(definition => group.locked
              ? <label className="lb-field" key={definition.key}><span>{definition.label}</span><select disabled value={definition.options.find(option => option.startsWith(form.grading?.[definition.key] + ' ')) || ''}><option value="">Awaiting faculty</option>{definition.options.map(option => <option key={option}>{option}</option>)}</select></label>
          ...
          <details className="lb-more" open={moreOpen} onToggle={(event) => setMoreOpen(event.currentTarget.open)}><summary><Plus size={15} /> Remarks, notes and photos <span>Optional</span></summary><div className="lb-form-grid">
            <label className="lb-field"><span>Remarks</span><textarea name="remarks" rows={3} maxLength={500} value={form.extra.remarks || ''} onChange={(event) => extraChange('remarks', event.target.value)} /><small>Up to 500 characters. Included in your submission.</small></label>
            <label className="lb-field"><span>Personal notes</span><textarea rows={3} value={form.extra.notes || ''} onChange={(event) => extraChange('notes', event.target.value)} maxLength={4000} /><small>Stay editable after approval. Not part of the signed record.</small></label>
        ...
        <LogbookAccordionSection index={2} title="Faculty verification" summary={form.faculty ? (FACULTY.find(person => person.id === form.faculty)?.name || 'Choose faculty') : 'Choose a reviewer and confirm your entry'} open={step === 2} complete={!Object.keys(validation).length} disabled={!contextReady} onOpen={() => { if (Object.keys(detailsErrors).length) revealErrors(detailsErrors); else openStep(2) }}>
          <label className="lb-field"><span>Verifying faculty <span className="lb-required" aria-hidden="true">*</span></span><select name="faculty" value={form.faculty} aria-required="true" aria-invalid={Boolean(errors.faculty)} aria-describedby={errors.faculty ? 'lb-faculty-error' : undefined} onChange={(event) => change('faculty', event.target.value)}><option value="">Select faculty</option>{FACULTY.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select>{errors.faculty && <small id="lb-faculty-error" className="lb-error">{errors.faculty}</small>}</label>
