...
export default function LogbookDashboard({ entries, onNavigate, onOpen, onSubject, onEdit, onRemedial }) {
  ...
  const metrics = [
    ...
    { label: 'Needs action', value: drafts.length + remedials.length, icon: FilePenLine },
  ]
  ...
  return <div className="lb-overview">
    <div className="lb-summary-strip" aria-label="Logbook summary">{metrics.map(({ label, value, icon }, index) => {
      ...
      return <div className={`lb-summary-item my-skills-metric-card ${['is-assigned', 'is-assigned', 'is-completed', 'is-completed', 'is-results', 'is-results'][index]}`} key={label}><span className="my-skills-metric-card-icon"><MetricIcon size={18} aria-hidden="true" /></span><span className="my-skills-metric-card-copy"><strong>{value}</strong><span>{label}</span></span></div>
    })}</div>
    <div className="lb-overview-top">
      ...
      <section className="lb-overview-panel"><div className="lb-section-head"><h2>Needs your action <span className="lb-overview-count">{drafts.length + remedials.length}</span></h2><button className="lb-text-btn" onClick={() => onNavigate('search')}>View all <ArrowRight size={14} /></button></div>
