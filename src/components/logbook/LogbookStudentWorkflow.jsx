...
export default function LogbookStudentWorkflow({ entries, onEdit }) {
  ...
  return <div className="lb-student-workflow">{tasks.length > 0 && <section className="lb-card lb-stack"><h2>Assigned to you</h2>{tasks.map(entry => <article className="lb-assigned-task" key={entry.id}><div><strong>{entryTitle(entry)}</strong><small>{entry.subject} / {categoryLabel(entry.cat)}</small>{entry.assignment.instructions && <p>{entry.assignment.instructions}</p>}<small>{actorName(entry.assignment.by)} | {entry.assignment.due ? `Due ${formatDate(entry.assignment.due)}` : 'No deadline'}</small></div><button className="lb-btn lb-primary" onClick={() => onEdit(entry)}>Complete entry</button></article>)}</section>}</div>
}
